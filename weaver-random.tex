...
struct _Wrng{
  ...
  short offset;       // 32-bit offset for the return value 
  @<Declaração de Mutex@>
};
#endif
@
...
memcpy(&tmp, aux, 16);
result = result ^ tmp;
rng -> index = (rng -> index + 1) % _N;
rng -> offset = (rng -> offset + 1) % 4;
((unsigned __int128 *) (rng -> w))[rng -> index] = result;
return (uint32_t) (result >> 96);
#else
...
uint32_t _Wrand(struct _Wrng *rng){
  if(rng -> offset == 0){
    ...
    if(rng -> offset == 3)
      rng -> index = (rng -> index + 1) % _N;
    rng -> offset = (rng -> offset + 1) % 4;
    return (uint32_t) ((result << (32 * (rng -> offset - 1))) >> 96);
  }
}
#endif
@
