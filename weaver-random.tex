...
Este teste é o primeiro teste empírico sugerido em [Knuth,
1998]. Nele, nosso objetivo é usar o gerador para simular a geração de
números entre 0 e 1 e checar se estatisticamente obtemos valores bem
espalhados ao longo do segmento.

O teste funciona gerando 1000 valores aleatórios de 32 bits e
aplicando o teste de Smirnov-Kolmogorov para obter medida do quão
próximo tais números estão espalhados em relação ao esperado. A medida
é repetida 30 vezes e seus diferentes resultados são novamente
comparados com a distribuição esperada novamente por meio de
Smirnov-Kolmogorov. Se o resultado é suficientemente improvável (1\%
de chance de estar diferente demais ou próximo demais do valor
idealizado), consideramos que o teste falhou. Se for algo improvável,
mas não tanto (5\% de chance de estar diferente demais ou próximo
demais do valor idealizado), damos uma penalidade à esta medida. Após
3 medidas se tivermos 2 penalidades ou somente 1 falha, consideramos
uma falha no teste. Por fim, o procedimento acima é repetido 10000
vezes e assim obtemos uma prcentagem de quantas falhas obtivemos.

Temos na verdade duas porcentagens de sucesso: uma para o teste
$K^+_n$, que detecta se há regiões que tem mais pontos gerados que o
esperado, e outra para o $K^-_n$ que detecta se há região com menos
pontos que o esperado.

Fizemos o experimento usando a semente 2613561388 gerada
aleatoriamente. Além disso, tanto para $K^+_n$ como para $K^-_n$
medimos a taxa de sucesso primeiro ao usar diretamente o valor
retornado pelo gerador e segundo ao inverter a ordem de seus
bits. Isso é feito porque este teste dá um peso maior para os bits
mais significativos, e é desejável arantir que os bits menos
significativos são tão bons quanto. O resultado foi:
...
Sucesso ($K^+_n$)\hidewidth&&
\omit\hidewidth Sucesso ($K^-_n$)\hidewidth&\cr\linha
&&SFMT&&91\%/92\%&&92\%/92\%&\cr\linha
&&Xorshiro**&&91\%/91\%&&92\%/93\%&\cr\linha
&&PCG&&91\%/93\%&&91\%/93\%&\cr\linha
&&\monoespaco{rand()}&&91\%/91\%&&91\%/92\%&\cr\linha
&&ChaCha20&&92\%/92\%&&91\%/92\%&\cr\linha}}

Todos os algoritmos testados apresentaram desempenho satisfatório
neste teste mais básico, independente de invertermos os bits ou
não. Para fins de referência, o valor esperado para uma distribuição
completamente aleatória é 92,34\% em todos os testes. Sucesso muito
acima disso faz a distribuição agir de maneira idealizada demais para
ser próxima de aleatória e sucesso muito abaixo denota que o gerador
tem uma qualidade baixa e diferente demais de algo aleatório.

\subsecao{3.1. Teste Serial}
